...
\IEEEauthorblockA{\textit{Teleinformatics Engineering Department} \\
...
\begin{figure}[htbp]
\centerline{\includegraphics[width=\columnwidth]{../figures/monovariate_histograms_0}}
\caption{Example of a figure caption.}
\label{fig}
\end{figure}

An interesting fact is that the components that, in general are not absent in the concrete composition has the lower skewness. These components are beyond Cement and Water, Coarse and Fine Aggregates. The other components have higher skewness due to the fact they are optional in concrete mixture, thus they allow to have zero values in some samples. Another seeable fact is the discreteness of the Age values. In general the concrete achieves the \emph{cure} at the end of 28 days. this explains the size of the bins between 0 and 100. The other cure periods are sampled in dataset to test its influence in compressive strenght.
